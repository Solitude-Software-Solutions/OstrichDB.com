...
const ProfileDropdown: React.FC = () => {
  ...
  const handleMenuClick = (action: string) => {
    setIsOpen(false);
    //transmport the user to the appropriate page based on the action
    if (action === 'projects') {
  ...
  const getTierStyle = (tier: string) => {
    const styles = {
      ...
      "Pro": { color: "#F59E0B" },
      //Premium tier uses purple becuase why not? 
  ...
  const displayName = userInfo?.givenName && userInfo?.familyName 
    ? `${userInfo.givenName} ${userInfo.familyName}`
    : userInfo?.givenName || 'User';
  
  const userEmail = userInfo?.email || '';
  const profilePhoto = userInfo?.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=random`;
