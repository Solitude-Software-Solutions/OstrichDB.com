    ...
    return (
      <div 
        className={`absolute top-full left-0 w-64 bg-sb-dark-accent rounded-md shadow-lg p-4 transform origin-top-left transition-all duration-200 ${
          activeDropdown === link.label ? 'animate-slide-down opacity-100' : 'opacity-0 invisible'
        }`}
      >
  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-sb-dark/90 backdrop-blur-sm shadow-lg' : 'bg-transparent'
      }`}
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <div key={link.label} className="relative">
                <button
                  onClick={() => link.isDropdown && toggleDropdown(link.label)}
      ...
      <div
        className={`md:hidden bg-sb-dark-accent absolute w-full transition-all duration-300 ease-in-out ${
          mobileMenuOpen ? 'max-h-[80vh] opacity-100' : 'max-h-0 opacity-0 invisible'
        } overflow-hidden`}
      >
